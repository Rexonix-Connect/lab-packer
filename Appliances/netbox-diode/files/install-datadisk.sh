...
# /srv/diode.
#
# Everything that grows lives here - Docker's data root, which is both the
# baked container images and every named volume (so the Diode PostgreSQL
# cluster and the Redis append-only file land on it), plus the backups - so
# the 60 GB root inherited from the hardened base cannot be filled by Diode's
# own data.
#
...
# the deploy wizard a hands-off operation (see diode-datadisk.service).
set -euo pipefail

MOUNTPOINT=/srv/diode
LABEL=diode-data
...
# RequiresMountsFor= drop-ins are what stop Docker and the stack starting
# without it, which is a visible failure rather than a silent one - a Docker
# that starts on an empty data root would quietly recreate empty volumes.
# nodev/nosuid match the hardened base's posture - but not noexec, because
# container layers are executed from here.
