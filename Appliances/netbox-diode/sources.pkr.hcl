source "vsphere-clone" "base" {
  ...
  # item, the same source the NetBox appliance clones. An OVF-backed content
  # library source inherits the base's firmware, disk layout and hardening;
  # the plugin rejects disk_size, storage and disk_controller_type for this
  # source type, so the 60 GB root disk comes across as-is and is grown at
  # deploy time instead (the base ships growpart and resize_rootfs for exactly
  # that).
  #
  ...
  # The eleven inherited properties come across with the clone and the Diode
