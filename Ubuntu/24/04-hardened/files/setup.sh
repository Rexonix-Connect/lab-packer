...
echo '> Verifying hardened-image extras ...'
...
for opt in nodev nosuid noexec; do
	case ",${shm_opts}," in
		*",${opt},"*) ;;
		*) echo "> /dev/shm is missing mount option ${opt} (have: ${shm_opts})"; exit 1 ;;
	esac
done
# Kernel lockdown is expected under Secure Boot; warn rather than fail so a
# deliberate firmware=bios/efi fallback still builds.
